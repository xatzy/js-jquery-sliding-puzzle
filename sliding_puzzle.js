...
class SlidingPuzzle {
    ...
    */
    ...
    /**
    * Returns the number of inversions of an integer i against a one dimension array of integer
    */
    _checkInversions(index, arr)
    {
        let count = 0;
    ...
    /**
    * Makes sure our puzzle is solvable
    */
    _checkPuzzleIsSolvable()
    {
        /* Will contain the result of ordering, so we can compute inversions
        * Example:
           1 2 3
           4 5 6
           8   7
        Will give 1 2 3 4 5 6 8 7 
        */
        ...
        */
        ...
        // Parses the board, maps each to a value in the one dimension array,
        // Then filters the blank and decreases by one each the value of piece that comes after the blank one
        for (var i = 0; i < this.pieces.length; i++)
            for (var j = 0; j < this.pieces[i].length; j++)
            {
                oneDimensionArray[i * this.divideBy + j] = this.pieces[i][j].startPosi * this.divideBy + this.pieces[i][j].startPosj;
                if (this.pieces[i][j].canvas == null)
                    emptyIndex = {row:i, index:i * this.divideBy + j, value:this.pieces[i][j].startPosi * this.divideBy + this.pieces[i][j].startPosj};
            }

        // Removes the blank piece
        oneDimensionArray.splice(emptyIndex.index, 1);
        // Decreases the value over blank piece by one
        oneDimensionArray = oneDimensionArray.map(
            x => x > emptyIndex.value ? x -= 1 : x);

        let inversions = 0;
        for (var i = 0; i < oneDimensionArray.length; i++)
        {
            inversions += this._checkInversions(i, oneDimensionArray);
        }
    ...
    _shufflePieces()
    {
        ...
        while (!this._checkPuzzleIsSolvable())
        {
            this.__shuffle();
        }
    }
    ...
    */
    ...
    /**
    * Gets a sub-image from the main image
    */
